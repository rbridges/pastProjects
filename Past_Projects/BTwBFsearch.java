...
public class BinaryTree<E> implements Serializable {
    ...
    public int maxHeight(Node<E> localRoot){
      if(localRoot == null){
        return 0;
      }
      
     return (1 + (Math.max(maxHeight(localRoot.right), maxHeight(localRoot.left)) ));
    }
    
        
    public int minHeight(Node<E> localRoot){
      if(localRoot == null){
        return 0;
      }
      
     return (1 + (Math.min(minHeight(localRoot.right), minHeight(localRoot.left)) ));
    }
    
    public String inorderStringBack(){
    ...
    private void inOrderBuilder(StringBuilder sb, LinkedList q){
      if (q.isEmpty()) return;
      Node<E> node = (Node<E>)q.poll();
      sb.append(node + " ");
